...
public class T0004_median_of_two_sorted_arrays {
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        int la = nums1.length;//数组 A 的长度
        int lb = nums2.length;//数组 B 的长度
        ...
        while (aMin <= aMax) {
            ...
            if (aIndex > aMin && nums1[aIndex - 1] > nums2[bIndex]) {
                aMax = aIndex - 1;
                //数组 A 分割点相邻右边那个元素比数组 B 分割点相邻左边那个元素大，则应该将数组 A 分割点向左移，数组 B 分割点向右移
                //数组 A 分割点有向右移趋势，需检查右边界
            } else if (aIndex < aMax && nums2[bIndex - 1] > nums1[aIndex]) {
                aMin = aIndex + 1;
                //得出结果
            } else {
                ...
                //情况一,情况二2，情况三2
                if (aIndex == 0) {
                    leftPart = nums2[bIndex-1];
                    //情况三1
                } else if (bIndex == 0) {
                    leftPart = nums1[la - 1];
                    //情况二1,情况二3,情况三3
                } else {
                    leftPart = Math.max(nums1[aIndex - 1], nums2[bIndex-1]);
                }
                ...
                //情况一,情况二1
                if (aIndex == la) {
                    rightPart = nums2[bIndex];
                    //情况三2
                } else if (bIndex == lb) {
                    rightPart = nums1[0];
                    //情况二2、3，情况三1、3
                }else {
                    rightPart = Math.min(nums1[aIndex], nums2[bIndex]);
                }
